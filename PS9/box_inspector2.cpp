...
BoxInspector2::BoxInspector2(ros::NodeHandle* nodehandle) : nh_(*nodehandle) { //constructor
    ...
    NOM_BOX2_POSE_WRT_WORLD.pose.position.y = -0.28;
...
bool BoxInspector2::update_inspection(
    ...
    int num_parts_seen = filtered_box_camera_image.models.size(); //Starts with 4
    int num_parts_desired = desired_models_wrt_world.size(); //Starts with 0 <- Should this be zero?
    ...
    if (get_bad_part_Q(bad_part,cam_num)) {
        ...
        cout<<"Bad part is: "<<bad_part.name<<endl;
        ...
        while ((!found)&&(ipart_seen < num_parts_seen)) {
            ...
            if (compare_pose(model_pose_from_image_wrt_world.pose, bad_part.pose.pose)) {
                ...
                cout<<"Bad part is now: "<<test_model<<endl; //Debug to print what type of defective part has been found
    ...
    //Checks remaining parts, seen as either misaligned or orphaned
